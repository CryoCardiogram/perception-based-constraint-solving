...
class NeurASP(object):
    ...
    def satisfy(model, asp):
        ...
        for atom in model:
            asp_with_facts += atom + '.\n'
        # print('debug clingo asp with facts?', asp_with_facts)
        clingo_control = clingo.Control(['--warn=none'])
            ...
            for dataIdx, data in iterator:
                ...
                for m in self.nnOutputs:
                    ...
                    for t in self.nnOutputs[m]:
                        ...
                        nnOutput[m][t] = self.nnMapping[m](dataTensor.to(self.device))['predictions']
                ...
                if alpha < 1:
                    ...
                    #print('step 2.1?')
                    for ruleIdx in range(self.mvpp['nnPrRuleNum']):
                        #print('rule?', ruleIdx, dmvpp.parameters[ruleIdx])
                    ...
                    if storeSM:
                        ...
                        if method == 'exact':
                            ...
                            #print('with nn_prediction')
                            models = dmvpp.find_one_most_probable_SM_under_obs_noWC()
                            #print('any models?', models)
                            #print(self.mvpp['program_asp'])
