...
def split(x, grid_shape, pad=0.0):
    ...
    padded_img = F.pad(
        x, (pad_w // 2, pad_w // 2, pad_h // 2, pad_h // 2), mode="replicate"
    )
    patches = padded_img.unfold(2, new_kernel_h, kernel_h).unfold(
        3, new_kernel_w, kernel_w
    )
    for cellimg in (
        patches.reshape(B, C, -1, new_kernel_h, new_kernel_w)
        .permute(0, 2, 1, 3, 4)
        .view(-1, C, new_kernel_h, new_kernel_w)
    ):
        yield cellimg
...
class GridSplitter(torch.nn.Module):
    ...
    def forward(self, img: torch.Tensor):
        ...
        padded_img = F.pad(
            img, (pad_w // 2, pad_w // 2, pad_h // 2, pad_h // 2), mode="replicate"
        )
        patches = padded_img.unfold(2, new_kernel_h, kernel_h).unfold(
            3, new_kernel_w, kernel_w
        )
        return self.cell_transforms(
            patches.reshape(B, C, -1, new_kernel_h, new_kernel_w)
            .permute(0, 2, 1, 3, 4)
            .view(B, -1, C, new_kernel_h, new_kernel_w)
        )
...
class MeshSplitter(torch.nn.Module):
    ...
    def __init__(
        self,
        grid_shape,
        cell_transforms: torch.nn.Module,
        meshgrid: Iterable[Iterable[Tuple[int, int]]],
    ) -> None:
    ...
    def forward(self, img: torch.Tensor):
        ...
        for mesh in self.meshgrid:
            ...
            # crop bbox
            yield self.cell_transforms(
                img.squeeze()[:, y : y + h, x : x + w]
            ).unsqueeze(0)
...
def extract_grid(img: np.ndarray):
    ...
    # PORTRAIT IMAGE
    if width < height:
        imageBGR_cropped = img[
            math.floor(height / 2 - width * 0.375) : math.ceil(
                height / 2 + width * 0.375
            ),
    ...
    # LANDSCAPE IMAGE
        ...
        imageBGR_cropped = img[
            math.floor(height * 0.125) : math.ceil(height - height * 0.125),
            math.floor(width / 2 - height * 0.375) : math.ceil(
                width / 2 + height * 0.375
            ),
            :,
        ]
...
class CorruptSudoku:
    ...
    def __init__(
        self, store: dict, num_errors=0, seed=None, smart=True, font_clf=True
    ) -> None:
    ...
    def make_sudoku_unsat(self, givens):
        ...
        has_more_than_one = cp.SolverLookup.get("ortools", m).solve(
        ...
        while count < self.num_errors:
            ...
            bv, bh = i // bs, j // bs
            options -= set(
                givens[bv * bs : (bv + 1) * bs, bh * bs : (bh + 1) * bs].flatten()
            )
            if len(
                options
            ):  # could be no options are left (unlikely, but still it can happen)
    ...
    def __call__(self, input_img: torch.Tensor, target):
        ...
        ), f"expected Tensor, got {type(input_img)}"
        ...
        try:
            ...
            if isinstance(e, FullSudokuException):
                ...
                print(
                    "cannot modify this instance",
                    target["label"].reshape(self.PUZZLE_SHAPE),
                )
                return input_img, target
            else:
                raise e
        ...
        instance[err_input] = (
            corrupted[err_input] + 9 if self.font_clf else corrupted[err_input]
        )
        new_target = {
            "label": torch.from_numpy(instance),
            "cs_output": target["cs_output"],
            "incorrect": np.array(list(zip(*err_input))),
        }
