...
from .datawrapper import CellDataset, ImgDataset, CellIndexedDataset, BalanceClassSampler
...
import numpy as np
from config import Datasets, BackboneArch, ImbalancePolicy, IS_MULTI, IS_WHOLE
...
class FullImageSudokuDataModule(pl.LightningDataModule):
    """Data module for (unlabeled) Visual Sudoku Puzzle
    ...
    """
    def __init__(self, datawrapper, data_dir:str = '.', conf=dict(), data_transforms=dict()) -> None:
        super().__init__()
        self.data_dir = Path(data_dir).resolve()
        assert (self.data_dir / 'img').is_dir(), f'bad data format, should be {data_dir}/img'
        assert (self.data_dir / 'label').is_dir(), f'bad data format, {data_dir}/label'
        self.datawrapper = datawrapper
        self.data_transforms = data_transforms
        self.conf = conf 
        self.is_cell_lvl_arch = conf.get('backbone', None) != BackboneArch.SUDOKUNET
        imbalance_policy = conf.get('imbalance_policy',None)
        self.is_stratified_sampling = imbalance_policy == ImbalancePolicy.DOWNSAMPLING or imbalance_policy == ImbalancePolicy.UPSAMPLING

        self.imgs_fname = [os.path.join(self.data_dir,'img',n) for n in os.listdir(self.data_dir / 'img')]#sorted(os.scandir(self.data_dir / 'img'), key=lambda dentry: dentry.stat().st_mtime)]
    ...
    def train_dataloader(self) -> TRAIN_DATALOADERS:
        ...
        x = [extract_grid(np.array(Image.open(f))) for f in self.imgs_fname][:self.conf['max_data_size_unlabeled']]
        # dummy labels
        y = np.random.randint(0, 10, (len(x), 9,9))[:self.conf['max_data_size_unlabeled']]
        dataset = self.datawrapper(x,y, solver=None, **self.data_transforms)

        # dataloaders kwargs
        sampler = RandomSampler(dataset)
        return DataLoader(dataset, batch_size=None if self.is_cell_lvl_arch else 1, sampler=sampler)

    def val_dataloader(self) -> EVAL_DATALOADERS:
        # to implement if training is required
        pass 
...
def get_data(dataset:Datasets, puzzle_shape, max_data_size:int=None, fontstyle:bool=False):
    ...
    if dataset == Datasets.VSUDOKU_APP:
        x = np.load(os.path.join(root_path,
                                 'data/raw_data/app/data.npy')).astype(np.uint8)
        y_num = torch.tensor(np.load(os.path.join(root_path,
                                                  f'data/raw_data/app/labels.npy')).reshape(-1, *puzzle_shape), dtype=torch.long)
        ...
        y_style = torch.tensor(np.load(os.path.join(
                root_path, f'data/raw_data/app/labels_hw.npy'
            )).reshape(-1, *puzzle_shape), dtype=torch.long)[max_data_ind]
        
        combined_labels = np.asarray([labels2int[num][hw] for num, hw in zip(y_num.reshape(-1).numpy(), y_style.reshape(-1).numpy())]).reshape(y_num.shape)
        ...
        img_dict = _load_pickle(os.path.join(root_path, 'data/raw_data/neurasp/image_dict_reg_100.p'))
        label_dict = _load_pickle(os.path.join(root_path, 'data/raw_data/neurasp/label_dict_reg_100.p'))
        sorted_keys = sorted(list(img_dict.keys()))
        x = np.array([Image.fromarray(img_dict[k]).resize((300,300)) for k in sorted_keys])
        ...
        assert os.path.exists(os.path.join(vizsudoku_symbolic_dir, 'features.pt')), 'generate data set first'
...
def get_data_loaders(raw_data_dict:dict, backbone: BackboneArch, solver: ConstraintSolver,
               kwargs_dataset_transforms: dict, conf: dict, seed_train_val = 244):
    ...
    train_index, test_index = train_test_split(np.arange(len(x)), train_size=0.7, random_state=243)
    train_index, valid_index = train_test_split(train_index, train_size=0.8, random_state=seed_train_val)
    ...
    if n_errors > 0:
        assert conf['dataset'] == Datasets.VSUDOKU_APP, f'Cannot build corrupted instances with dataset {conf["dataset"]}'
    ...
    imbalance_policy = conf.get('imbalance_policy', None)
    is_stratified_sampling = imbalance_policy == ImbalancePolicy.DOWNSAMPLING or imbalance_policy == ImbalancePolicy.UPSAMPLING
    ...
    if sols is None:
        ...
        with ProcessPoolExecutor(max_workers=conf['n_workers']) as executor:
            sols_list = list(tqdm(executor.map(solver.solve, y, chunksize=100), total=len(y)))
        dict_of_lists_of_tensors = {key: [torch.from_numpy(d[key]).float() if isinstance(d[key], np.ndarray) else torch.tensor([d[key]]) 
                                          for d in sols_list] 
                                          for key in sols_list[0].keys()}
        
        sols = {key: torch.stack(tensor_list) if isinstance(tensor_list[0], torch.Tensor) else tensor_list 
        ...
        torch.save(sols, os.path.join('data', 'raw_data', folder_name[conf['dataset']], 'sols.pt'))
    ...
    for i, index in enumerate([train_index, valid_index, test_index]):
        ...
        if i > 0:
            ...
            data_transforms = {k:v for k,v in filter(filter_transforms, kwargs_dataset_transforms.items())}
        if b_augment_sudoku or b_corrupt_sudoku:
            assert conf['dataset'] == Datasets.VSUDOKU_APP, 'sudoku augment only available for Sudoku Assistant App Dataset'
            assert 'combined_labels' in raw_data_dict, 'missing combined labels to build corrupted instances'
            # build corrupter
            ...
            store = {
                ...
                k: data_store.cells_img[data_store.labels == k+9,:,:] for k in range(1,10)
            }

        if i == 2:
            if b_corrupt_sudoku:
                corrupter = CorruptSudoku(store, num_errors=n_errors, seed=seed_train_val, font_clf=conf.get('fontstyle', False))
                # filter out instances that don't have empty spaces
                data_transforms['corrupter'] = corrupter
        elif i == 1:
            if conf.get('corrupt_valid', False):
                corrupter = CorruptSudoku(store, num_errors=n_errors, seed=seed_train_val)
                # filter out instances that don't have empty spaces
                data_transforms['corrupter'] = corrupter
        else: #i == 0:
            pass
        ...
        if is_stratified_sampling and  i == 0 and is_cell_lvl_arch:
            ...
            list_data.append( 
                CellIndexedDataset(
                    ...
                    solver,
                    shape=conf['puzzle_shape'],
                    sols=None if sols is None else {k:v[index] for k,v in sols.items()} ,
                    **data_transforms
                )
            )
        else:
    ...
    def collate_subsampling(list_of_patch_target):
        ...
        x = torch.vstack([make_grid(torch.vstack(patches[i:i+81]), nrow=9, padding=0) for i in range(0, len(patches), 81)])
        target = {
    ...
    if is_stratified_sampling and is_cell_lvl_arch:
        sampler = BalanceClassSampler(list_data[0].y.reshape(-1).numpy().tolist(), imbalance_policy.value.lower())
    ...
    if is_stratified_sampling:
        ...
        trainloader = DataLoader(train_data, batch_size=train_batch_size, sampler=sampler, num_workers=n_max_workers, drop_last= train_batch_size is not None)

    validloader = DataLoader(valid_data, batch_size= 1)
    testloader = DataLoader(test_data, batch_size= 1, shuffle=False)
    return trainloader, validloader, testloader

def build_data(dataset: Datasets, backbone: BackboneArch, solver: ConstraintSolver,
               kwargs_dataset_transforms: dict, conf: dict, seed_train_val = 244):
    raw_data_dict = get_data(dataset, conf['puzzle_shape'], conf['max_data_size'], conf.get('fontstyle', False))
    return get_data_loaders(raw_data_dict, backbone, solver, kwargs_dataset_transforms, conf, seed_train_val)
