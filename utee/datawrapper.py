...
def add_augment_before_cast_transform(
    transforms: List, augment: torch.nn.Module
) -> List:
...
class ImgDataset:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.y = y
        try:
            tf_list, idx = add_augment_before_cast_transform(
                img_transform.transforms, img_augment
            )
        ...
        if solver is not None and kwargs.get("sols", None) is None:
            self.sols = [solver.solve(yi, **p) for yi, p in zip(y, solver_params)]
        else:
            if "sols" in kwargs:
                self.sols = [
                    {k: v[i] for k, v in kwargs["sols"].items()}
                    for i in range(len(kwargs["sols"]["runtime"]))
                ]
            else:
...
class CellDataset:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        list_img_transforms, _ = add_augment_before_cast_transform(
            img_transform.transforms, img_augment
        )
        ...
        normalize = (
            list_img_transforms[index_normalize:]
            if self.has_normalize_tf
        ...
        self.cell_transform = T.Compose(
            [T.ToPILImage(), cell_transform, cell_augment, T.ToTensor(), *normalize]
        )
    ...
    @lru_cache(
        30
    )  # use LRU to cache whole img in memory. This speeds up training considerably
    def __getitem_solo__(self, index):
        ...
        target = {"label": self.y[index].flatten(), "cs_output": self.sols[index]}
        whole_img, target = self._transform_img(self.X[index], target)
        img_idx = torch.zeros(*target["label"].shape)
        img_idx[:] = index
        target["img_idx"] = img_idx
        cell_imgs = [
            self.cell_transform(cellimg)
            for cellimg in split(whole_img.unsqueeze(0), self.shape, self.overlap_pad)
        ]
