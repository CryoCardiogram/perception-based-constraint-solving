...
class PBCSweightedCE(PBR):
    ...
    def compute_loss(self, dnn_output: dict, cs_output, target) -> dict:
        ...
        classes_counts = torch.tensor(
            [counter.get(k, 1) for k in range(self.num_pred_classes)],
            dtype=torch.float,
            device=self.device,
        )
        train_class_weights = 1 - (classes_counts / classes_counts.sum())
        weighted_bce = nn.BCELoss(
        ...
        cell_value_loss = weighted_bce(
            ...
            torch.eye(self.num_pred_classes, device=self.device)[
                target["label"].view(*target_dim)
            ].float(),
        )

        return {"cell_value_ce_loss": cell_value_loss}
