...
from config import CalibrationType, BackboneArch, IS_MULTI
...
class Calibration(PBR):
    ...
    def __init__(self, dnn: nn.Module, constraint_solver: ConstraintSolver, splitter:nn.Module=nn.Identity(),
                 lr=0.01, num_pred_classes=10, puzzle_shape=..., hparams=..., calib_dl=None):
        super().__init__(dnn, constraint_solver, splitter, lr, num_pred_classes, puzzle_shape, hparams)
        #assert hasattr(dnn, 'calibration'), 'missing calibration wrapper'
    ...
    def forward(self, x, solve=False):
        ...
        if self.calibrated and self.calibration_method != CalibrationType.DIRICHLET:
            for head in self.dnn.calibration.keys():
                dnn_output[head] = F.softmax(self.dnn.calibration[head](dnn_output['logits'][head].reshape(B*n_item, -1)), dim=1)
        elif self.calibrated and self.calibration_method == CalibrationType.DIRICHLET:
            pass
            
        elif not 'predictions' in dnn_output:
            for head in self.dnn.calibration.keys():
                dnn_output[head] = F.softmax(dnn_output['logits'][head].reshape(B*n_item, -1), dim=1)
        #print(dnn_output)
        # print('DEBUG PREDICTIONS ?', dnn_output['predictions'].reshape(B, self.n_items, -1)[:,0])
        # print('DEBUG PREDICTION ROWSUM? ', dnn_output['predictions'].reshape(B, self.n_items, -1)[:,0].sum())
        cs_output = None 
        if solve:
            cs_output = self.constraint_solver.solve_from_torch(
                dnn_output['predictions'].reshape(B, *self.puzzle_shape, -1), self.device)
    ...
    def compute_loss(self, dnn_output: dict, cs_output, target) -> dict:
        ...
        if 'logits' in dnn_output:
            cell_value_loss = self.nll_criterion(
                dnn_output['logits']['predictions'].view(*target_dim, -1), target['label'].flatten()
            ) 
        else:
            cell_value_loss = self.bce_criterion(
                dnn_output['predictions'].view(*target_dim, -1), torch.eye(self.num_pred_classes)[target['label'].flatten()]
            )
    ...
    def on_test_end(self) -> None:
        ...
        _, confhist = ConfidenceHistogram().plot(self.outputs, self.labels, logits=False)
        ...
        for l in range(self.num_pred_classes):
            ...
            self.outputs = self.outputs.reshape(-1, self.outputs.shape[-1])
            _, reldia = ReliabilityDiagram().plot(self.outputs[idx], self.labels[idx], logits=False)
            reldia.savefig(path / f'rel_dia_label{l}.png')
            _, confhist = ConfidenceHistogram().plot(self.outputs[idx], self.labels[idx], logits=False)
    ...
    def _shared_eval(self, batch, batch_idx, log=True, testing=False):
        ...
        eval_output = super()._shared_eval(batch, batch_idx, log=False, testing=testing,
                                           pre_dnn_output=dnn_output, pre_cs_output=cs_output, num_classes=self.num_pred_classes)
        # calibration metrics
        cali_output = dict()
        ...
        _, ece, ece_cw, mce, _, brier = evaluate_slim(dnn_output['predictions'].cpu().numpy().reshape(n_items, -1), target['label'].cpu().numpy().flatten())
        ...
        if testing:
            if not hasattr(self, 'outputs'):
                self.outputs = []
                self.labels = []
            self.outputs.append(dnn_output['predictions'].detach().cpu().numpy().reshape(n_items, -1))
            self.labels.append(target['label'].cpu().numpy())

            target_shape = target['label'].flatten().shape
            labels_reduce = target['label'].reshape(*self.puzzle_shape, -1).reshape(*target_shape)
            pred_solve = torch.argmax(dnn_output['predictions'].reshape(*target_shape, -1), -1)
            gridacc = (cs_output['perception'].reshape(*target_shape) == labels_reduce).all().sum()
            if gridacc < 1:
                debug_save_cellimgs(x, pathlib.Path(self.logger.log_dir, f'not-solved_{batch_idx}'))
                torch.save(pred_solve, pathlib.Path(self.logger.log_dir,f'not-solved_{batch_idx}.pt' ))
...
class CalibrationOnly(Calibration):
    ...
    def __init__(self, dnn: nn.Module, constraint_solver: ConstraintSolver, splitter:nn.Module=nn.Identity(),
                 lr=0.01, num_pred_classes=10, puzzle_shape=..., hparams=..., calib_dl=None):
        super().__init__(dnn, constraint_solver, splitter, lr, num_pred_classes, puzzle_shape, hparams, calib_dl)
        ...
        self.is_dirichlet_on_cell_mtl = hparams['calibration'] == CalibrationType.DIRICHLET and hparams['backbone'] in IS_MULTI 
        self.dnn = get_shared_calibration_wrapper(self.calibration_method, self.dnn, self.num_pred_classes)#19 if self.is_dirichlet_on_cell_mtl else self.num_pred_classes)
    ...
    def training_step(self, batch, batch_idx):
        ...
        target_shape = target['label'].flatten().shape
        if self.is_dirichlet_on_cell_mtl:
            assert self.num_pred_classes > 10
            # apply dirichlet on merged output
            pred_merged = torch.zeros(B, self.n_items, 19, device=self.device).float()
            pred_digits = dnn_output['logits']['predictions'].reshape(B, self.n_items, -1).detach()
            pred_is_hw = dnn_output['logits']['styles'].reshape(B, self.n_items).detach()
            for b in range(B):
            ...
            dnn_output['logits']['predictions_merged'] = pred_merged
            return self.nll_criterion(self.dnn.calibration['predictions'](dnn_output['logits']['predictions_merged'].reshape(*target_shape, -1)), target['label'].reshape(target_shape))
        loss = self.nll_criterion(self.dnn.calibration['predictions'](dnn_output['logits']['predictions'].reshape(*target_shape, -1)), target['label'].reshape(target_shape))
        if self.hparams_initial['backbone'] in IS_MULTI:
            # build target
            label_style = (target['label'] >= 10).type(torch.float)
            # add loss term for style clf head
            loss += self.bce_logits(self.dnn.calibration['styles'](dnn_output['logits']['styles'].reshape(*target_shape, -1)), label_style.reshape(*target_shape, -1))
        return loss
    def configure_optimizers(self):
        print('debug config opt: ', self.calibration_method)
        if self.calibration_method == CalibrationType.TEMPERATURE :
            optimizer = torch.optim.LBFGS(self.dnn.calibration.parameters(), lr=self.lr, max_iter=500)
