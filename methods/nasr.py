...
class NASRTrainer(PBR):
    def __init__(
            self, dnn: nn.Module, constraint_solver: ConstraintSolver, splitter:nn.Module,
            lr=0.01, num_pred_classes=10, puzzle_shape=..., hparams=..., 
            freeze_level=0, weight_decay=3e-2, warmup=10, clip_gradient_norm=0
        ):
        """_summary_
    ...
    def forward(self, x, solve=False):
        ...
        if solve:
            ...
            cleaned_boards = torch.multiply(dnn_output['solver_nn_output'].view(B,self.n_items,-1),sampled_mask_boards.view(B, self.n_items, -1))
            dnn_output['cleaned_board'] = cleaned_boards
            cleaned_board_for_prob_solving = torch.zeros(B, self.n_items, dnn_output['solver_nn_output'].shape[-1]+1, device=self.device )
            ...
            # solver 
            cs_output = self.constraint_solver.solve_from_torch(cleaned_board_for_prob_solving, self.device, **dnn_output)
    ...
    def compute_loss(self, dnn_output: dict, cs_output:dict, target:dict) -> dict:
        ...
        final_solution = []
        for symbolic_solution, status, nn_solution  in zip(cs_output['solution'], cs_output['status'], dnn_output['cleaned_board']):
        ...
        rewards = (rewards - rewards.mean()) / (rewards.std() + torch.finfo(torch.float32).eps)
        # compute policy loss
    ...
    def configure_optimizers(self):
        optimizer = torch.optim.AdamW( 
            self.tf_sudoku.parameters(),
            lr = self.lr,
            weight_decay=self.weight_decay
        )
        scheduler = torch.optim.lr_scheduler.LinearLR(optimizer, start_factor=1/self.warmup, total_iters=self.warmup )
        return {
            'optimizer':optimizer,
