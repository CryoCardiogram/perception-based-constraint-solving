...
class PBR(pl.LightningModule):
    ...
    def __init__(self, dnn: nn.Module, constraint_solver: ConstraintSolver, splitter:nn.Module=nn.Identity(),
                 lr=1e-2, num_pred_classes=10, puzzle_shape=(9, 9), hparams=dict() ):
        super().__init__()
        self.dnn = dnn
    ...
    def _shared_eval(self, batch, batch_idx, log=True, testing=False, pre_dnn_output=None, pre_cs_output=None, num_classes=None):
        x, target = batch
        x = x.view(-1, *x.shape[-3:])
        B = len(x)
        ...
        # if eval_output[f'{str_eval_type}_grid_accuracy'] < 0.5:
        #     print('predictions', dnn_output['predictions'].reshape(B, self.n_items,-1).argmax(-1))
        #     print('labels', target['label'].reshape(B, -1))

        if testing:
            # handle unsat differently
            eval_output['unsolved'] = (cs_output['status'] == ExitStatus.UNSATISFIABLE.value).float().sum()# torch.tensor([1 if item == ExitStatus.UNSATISFIABLE.value else 0 for item in cs_output['status']])
            # if log and eval_output[f'{str_eval_type}_grid_accuracy_solve'] < 1:
            #     print(
            #         'pred', torch.argmax(
            #             dnn_output['predictions' if self.num_pred_classes > 10 else 'predictions_merged'].reshape(
            #                 B,self.n_items, -1), -1).reshape(B,
            #             *self.puzzle_shape),
            ...
            #                 *self.puzzle_shape), sep='\n')
                ...
                #             pathlib.Path(
                #                 self.logger.log_dir,
                #                 f'test_{batch_idx}_solved_{eval_output["test_grid_accuracy_solve"]}'))
            if self.logger is not None:
                torch.save(dnn_output, pathlib.Path(self.logger.log_dir,f'test_{batch_idx}_solved_{eval_output["test_grid_accuracy_solve"]}.pt' ))
                torch.save(target, pathlib.Path(self.logger.log_dir, f"target_{batch_idx}.pt"))    
    ...
    def configure_optimizers(self):
        # print('debug pbr_module: trained params:', [n for n, p in self.dnn.named_parameters() if p.requires_grad])
