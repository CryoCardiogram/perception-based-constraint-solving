...
def evaluate(probs, y_true, verbose=False, normalize=True, bins=15):
    ...
    ece2 = ECE(probs, y_true, bin_size=1/bins,
               ece_full=True, normalize=normalize)
    ...
    mce2 = MCE(probs, y_true, bin_size=1/bins,
               ece_full=True, normalize=normalize)
...
def softmax(x):
    ...
    """
    e_x = np.exp(
        x - np.max(x))  # Subtract max, so the biggest is 0 to avoid numerical instability
...
def compute_acc_bin_legacy(conf_thresh_lower, conf_thresh_upper, conf, pred, true):
    ...
    """
    filtered_tuples = [x for x in zip(
        pred, true, conf) if x[2] > conf_thresh_lower and x[2] <= conf_thresh_upper]
    if len(filtered_tuples) < 1:
        ...
        avg_conf = sum([x[2] for x in filtered_tuples]) / \
            len_bin  # Avg confidence of BIN
...
def compute_acc_bin(conf_thresh_lower, conf_thresh_upper, conf, pred, true, ece_full=False):
    ...
    Returns:
        (accuracy, avg_conf, len_bin): accuracy of bin, confidence of bin and number of elements in bin.
    """
    filtered_tuples = [x for x in zip(pred, true, conf) if (
        x[2] > conf_thresh_lower or conf_thresh_lower == 0) and x[2] <= conf_thresh_upper]
...
def binary_ECE(probs, y_true, power=1, bins=15):
    ...
    def bin_func(p, y, idx): return (
        np.abs(np.mean(p[idx]) - np.mean(y[idx])) ** power) * np.sum(idx) / len(probs)
...
def classwise_ECE(probs, y_true, power=1, bins=15):
    ...
    if not np.array_equal(probs.shape, y_true.shape):
        y_true = label_binarize(
            np.array(y_true), classes=range(probs.shape[1]))
...
def full_ECE(probs, y_true, bins=15, power=1):
    ...
    for bin in filled_bins:
        ...
        s += (len(i)/n) * (
            np.abs(np.mean(probs[i], axis=0) -
                   np.mean(y_true[i], axis=0))**power
        ).sum()
