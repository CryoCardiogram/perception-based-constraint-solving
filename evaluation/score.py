...
def get_cell_scorer(metric_cls, name):
    @torch.no_grad()
    def metric_scores(
        dnn_output: dict, num_classes, target: dict, cs_output: dict = None
    ):
        ...
        # before solve
        cell_metric = get_score(
            dnn_output["predictions"].detach().reshape(*target_shape),
        ...
        if cs_output is not None:
            cell_metric_solve = get_score(
                cs_output["perception"].reshape(*target_shape),
                target["label"].reshape(*target_shape),
            )
...
def grid_accuracy_scores(
    ...
    preds = (
        dnn_output["predictions"].detach().reshape(*target_shape, -1).argmax(-1).float()
    )

    grid_acc_val = (
        torch.eq(preds, target["label"].reshape(*target_shape)).all(-1).float()
    )
    res = {"grid_accuracy": grid_acc_val.mean()}
    if cs_output is not None:
        ...
        grid_acc_val_solve = (
            torch.eq(
                cs_output["solution"].reshape(*target_shape).long(),
                target["cs_output"]["solution"].reshape(*target_shape).long(),
            )
            .all(-1)
            .float()
        )
...
def cell_accuracy_scores(
    ...
    res = {"cell_accuracy": cell_acc_val}
    if cs_output is not None:
