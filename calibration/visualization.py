...
class ConfidenceHistogram(MaxProbCELoss):
    ...
    def plot(self, output, labels, n_bins = 15, logits = False, title = None):
        ...
        #plot histogram
        ax.hist(self.confidences,n_bins,weights = w,color='b',range=(0.0,1.0),edgecolor = 'k')
...
class ReliabilityDiagram(MaxProbCELoss):
    ...
    def plot(self, output, labels, n_bins = 15, logits = False, title = None, gap_message=''):
        ...
        ax.bar(x, self.bin_acc, color = 'b', width=delta,align='edge',edgecolor = 'k',label='Observed accuracy',zorder=5)
        ...
        ax.bar(x, error, bottom=np.minimum(self.bin_acc,mid), color = 'mistyrose', alpha=0.5, width=delta,align='edge',edgecolor = 'r',hatch='/',label=f'Gap{gap_message}',zorder=10)
        ident = [0.0, 1.0]
