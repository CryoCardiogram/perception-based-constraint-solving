...
def make_fc_layers(cfg, in_dim=512, activ=nn.ReLU, p=0.0, bn=False, out=nn.Softmax):
    ...
    for i, dim in enumerate(cfg):
        ...
        if i < len(cfg) - 1:
            layers += (
                [nn.BatchNorm1d(dim_out), activ(), nn.Dropout(p)]
                if bn
                else [activ(), nn.Dropout(p)]
            )
            continue
...
def make_fc_layers_global_pooling(
    in_dim=512, out_shape=(9, 9), num_classes=9, pooling=None, **pooling_args
):
    ...
    class AveragePooling(torch.nn.Module):
        def __init__(self) -> None:
            ...
            self.conv1x1 = torch.nn.Conv2d(
                in_channels=in_dim, out_channels=num_classes, kernel_size=1
            )
            self.global_pooling = (
                torch.nn.AdaptiveAvgPool2d(out_shape)
                if pooling is None
                else pooling(**pooling_args)
            )
