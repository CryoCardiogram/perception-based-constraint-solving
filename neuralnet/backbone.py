...
def mnist_feature_extract_layers(frozen=True, reset=False):
    pretrain_weights_path = os.path.join(root_path, "weights/plattcalibrated-matrix_mnist_e15_lr1e-05_tl0.01.pt")
    print(f'debug backbone mnist fe frozen {frozen}, reset {reset}')
    ...
    layers = [
        lenet.conv1, torch.nn.ReLU(), torch.nn.MaxPool2d(2,2), 
        lenet.conv2, torch.nn.ReLU(), torch.nn.MaxPool2d(2,2),
        torch.nn.Flatten()
    ]
    if reset:
...
def simclr_feature_extract_layers(frozen=True):
    ...
    weights = {'.'.join(k.split('.')[1:]):v for k,v in state_dict.items() if 'encoder' in k}
