...
from custom_types import IS_WHOLE, BackboneArch, CalibrationType, Datasets
...
SaltAndPepper = T.Compose([T.PILToTensor(), gauss_noise_tensor, T.ToPILImage()])
...
sudoku_assistant_raw_norms = T.Normalize(
    mean=[0.5320, 0.5209, 0.5204], std=[0.1862917, 0.19031495, 0.18998064]
)
...
normalizes = {
    ...
    Datasets.VSUDOKU_NEURASP: T.Normalize([0.5], [0.5]),
}
...
cell_tsfms = {
    ...
    BackboneArch.IMAGENET_RN18: T.Compose(
        [T.Resize((252, 252)), T.CenterCrop((224, 224))]
    ),
    ...
    BackboneArch.LENET: T.Compose(
        [T.Grayscale(num_output_channels=1), T.Resize((28, 28))]
    ),
...
cell_data_augments = {
    Datasets.VSUDOKU_APP: T.AutoAugment(
        T.AutoAugmentPolicy.SVHN
    ),  # T.RandomResizedCrop(28, scale=(0.95,1)),
    Datasets.VSUDOKU_MNIST: T.RandomResizedCrop(28, scale=(0.95, 1)),
    Datasets.VSUDOKU_NEURASP: T.RandomResizedCrop(
        28, scale=(0.95, 1)
    ),  # torch.nn.Identity()
}

img_data_augments = {
    Datasets.VSUDOKU_APP: T.ColorJitter(
        contrast=0.5, saturation=0.3, brightness=0.3, hue=0.25
    ),
...
feat_extractors_builder_outdim = {
    ...
    BackboneArch.LENET: (
        mnist_feature_extract_layers,
        800,
        {"frozen": False, "reset": True},
    ),
...
def get_model(
    ...
    if backbone in _multi_to_single.keys():
        ...
        backbone_builder, dim, kwargs = get_backbone_builder_outdim(
            _multi_to_single[backbone], frozen=frozen
        )
        # backbone
        ...
        backbone_builder, dim, kwargs = get_backbone_builder_outdim(
            _backbone_full_image_to_arch[backbone], frozen=frozen
        )
        feature_ext_layers = torch.nn.Sequential(*backbone_builder(**kwargs))
        # digit classifier head
        output_layer = make_fc_layers(
            fc_hidden_layers + [81 * N_CLASSES],
            in_dim=dim,
            out=nn.Identity,
            bn=batchnorm,
            p=dropout,
        ...
        base_img_tf.transforms.append(img_tsfms[backbone])
        ...
        print("(get_model) assemble all layers")
        net = CellNet(fe, fc)
    ...
    kwargs_dataset = {
        ...
        "cell_augment": cell_data_augments[dataset]
        if use_cell_augments
        else nn.Identity(),
        "img_transform": base_img_tf,
        "img_augment": img_data_augments[dataset]
        if use_img_augments
        else nn.Identity(),
    }
    # TODO refactor
    if use_cell_augments and backbone in [
        BackboneArch.LENET,
        BackboneArch.MNIST_LENET,
        BackboneArch.LENET_MULTI,
    ]:
    ...
    if use_cell_aug_at_img:
        kwargs_dataset["img_augment"] = T.Compose(
            [
                T.ToPILImage(),
                cell_data_augments[dataset]
                if dataset == Datasets.VSUDOKU_APP
                else T.RandomResizedCrop(252, scale=(0.95, 1)),
                T.ToTensor(),
            ]
        )
...
def get_shared_calibration_wrapper(
    calibration: CalibrationType, shared_net: SharedPatchNet, output_dim: int
):
...
def get_calibration_wrapped_model(
    calibration: CalibrationType, net: torch.nn.Module, output_dim: int
):
