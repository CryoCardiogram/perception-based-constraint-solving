...
def create_output_layer(output_layer_type:Enum, output_layer_config:list, n_items:int, enable_batch_norm:bool, num_pred_classes:int):
    ...
    if output_layer_type is None: 
        return nn.Sequential(
            Rearrange('batch (n_item n_class) -> (batch n_item) n_class', n_class=num_pred_classes),
            nn.Softmax(-1),
            Rearrange('(batch n_item) n_class -> batch n_item n_class', n_item=n_items)
        )
    if output_layer_type.value == "CORR":
        return nn.Sequential(
            *make_fc_layers(output_layer_config + [n_items * num_pred_classes],in_dim=n_items * num_pred_classes, bn=enable_batch_norm, p=.2, out=nn.Identity), 
            Rearrange('b (n_items n_pred_class) -> b n_items n_pred_class', n_items=n_items, n_pred_class=num_pred_classes),
            nn.Softmax(-1)
        )
    elif output_layer_type.value == "SHARED":
        return nn.Sequential(
            Rearrange('batch (n_item n_class) -> (batch n_item) n_class', n_class=num_pred_classes),
            *make_fc_layers(output_layer_config + [num_pred_classes],in_dim=num_pred_classes, p=0.2, bn=enable_batch_norm, out=nn.Identity), 
            Rearrange('(b n_items) n_pred_class -> b n_items n_pred_class', n_items=n_items),
            nn.Softmax(-1)
        )
        ...
        class PatchSpecificOutputNN(nn.Module):
            def __init__(self):
                ...
                self.out_layers = nn.ModuleList([nn.Sequential(*make_fc_layers(output_layer_config + [num_pred_classes], in_dim=num_pred_classes, p=0.2, out=nn.Identity)) for _ in range(n_items)])
            def forward(self, x):
