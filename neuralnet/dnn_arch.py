"""
This file defines deep neural network (DNN) architecture used as perception layers in the hybrid PBCS system.

DNN can operates at the single-item level (cell) or at the multi-item level (whole image), which is reflected by n.
...
from .generic_layers import make_conv_layers, make_fc_layers, make_fc_layers_global_pooling
...
class CellStyleNet(CellNet):
    ...
    def __init__(self, feat_ext: nn.Module, classifier: nn.Module,
                 style_clf: nn.Module = nn.Identity()) -> None:
        """
...
class PlattCellNet(CellNet):
    ...
    def __init__(self, feat_ext: nn.Module, classifier: nn.Module,
                 calibration_layer: nn.Module, style_clf:nn.Module = nn.Identity(),
                 style_calibration:nn.Module = nn.Identity(), **kwargs):
        super().__init__(feat_ext, classifier)
        ...
        self.calibration = nn.ModuleDict({'predictions':self.calibration_digit, 'styles':self.calibration_style})
...
class DirichletWrapper(nn.Module):
    """Dirichlet calibration wrapper for any NN module.

    `calibration` layer learns a mapping from softmax output to calibrated probabilities
    """

    def __init__(self, dnn: nn.Module, calibration_layer: nn.Module, **kwargs):
        """_summary_

        Args:
            dnn (nn.Module): _description_
            calibration_layer (nn.Module): _description_
        """
        super(DirichletWrapper, self).__init__()
        self.dnn = dnn
        self.calibration = nn.ModuleDict({'predictions':calibration_layer, 'styles':nn.Identity()})


        #self.style_clf = style_clf

    def logits(self, x):
        """Required for calibration learning loop.

        return the input of the calibration layer (in this case, output of the DNN)
        """
        return self.dnn(x)
    ...
    def forward(self, x):
        ...
        res['predictions'] = self.calibration['predictions'](logits_output['predictions'])
        return res
...
class SharedPatchNetCal(SharedPatchNet):
    ...
    def forward(self, x):
        ...
        if any(torch.isnan(p).any() for p in self.parameters()):
            ...
            print('following layers are impacted:', [n for n,p in self.named_parameters() if torch.isnan(p).any()])
        output_dict = {
            'logits':logits_output, # B x (0*K)
            'predictions':self.calibration['predictions'](h).softmax(-1),
        }
        if 'styles' in logits_output:
            output_dict['styles'] = self.calibration['styles'](logits_output['styles']).sigmoid()
...
class CellCNNThenAggregate(FullImageCNN):
    def __init__(self, backbone_layers: list, output_layers: list, aggregator:nn.Module) -> None:
        super().__init__(backbone_layers, output_layers)
        self.agg = aggregator
